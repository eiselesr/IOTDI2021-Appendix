...
class PlatformClient():
    ...
    def platformConnect(self, manager_ip, geth_ip, geth_port,index):
        ...
        self.contract_address=self.query_contract_address()
        self.ethclient = EthereumClient(ip=geth_ip, port=geth_port)
        self.getEthAccount(index)
        self.contract = ModicumContract.ModicumContract(self.ethclient, self.contract_address, index)
    ...
    def getReceipt(self, transactionHash):
        receipt = self.ethclient.command("eth_getTransactionReceipt", params=[transactionHash])
        self.logger.info("gasUsed: %s" %receipt['gasUsed'])
        self.logger.info("cumulativeGasUsed: %s" %receipt['cumulativeGasUsed'])
